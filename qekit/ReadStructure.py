...
def Decompose_POSCAR(filepath):
    ...
    if not os.path.exists(filepath):
        ...
        sys.exit()
    try:
        Lattice_Vector = []; Lattice_Constant = []; solver_ve = []
        ...
        for item in range(3, 6):
            ...
            Lattice_Constant.append(round(math.sqrt(float(vector[0]) ** 2 + float(vector[1]) ** 2 + float(vector[2]) ** 2), 5))
            if item == 4:
        ...
        alphe_calc = round(math.acos(((float(solver_ve[2]) / float(Lattice_Constant[2])) * math.sin(gamma_calc / 180 * math.pi)) + (math.cos(beta_calc / 180 * math.pi) * math.cos(gamma_calc / 180 * math.pi))) * 180 / math.pi, 4)
        Crystalline_Angle = {'gamma': gamma_calc, 'beta': beta_calc, 'alphe': alphe_calc}
        
        @staticmethod   # 该方法用于提取原子数量和种类信息
        def get_atom_info():
        ...
        @staticmethod  # 该方法用于POSCAR结构格式是D还是C，是否被固定
        def jug_fix():
        ...
        def Atom_Get_ElementLable_info(Atom_info,fix_info):
            Output_Atominfo_list = []; Output_Atominfo_list_step = []
            ...
            for jes in range(len(fix_info)):
                ...
                if jug_txt in check_isrepeat:
                    ...
                    Repeat_Number = fix_info[jes][1]; Repeat_Pos = fix_info[jes][2:]
                    fix_info[Repeat_Element_First_POS][1] = int(fix_info[Repeat_Element_First_POS][1]) + int(Repeat_Number)
        ...
        def calculate_Volume(a, b, c, alpha, beta, gamma):
            ...
            Volume = round(a * b * c * math.sqrt(1 - cos_alpha ** 2 - cos_beta ** 2 - cos_gamma ** 2 + 2 * cos_alpha * cos_beta * cos_gamma), 6)
            return Volume
        Volume = calculate_Volume(Lattice_Constant[0], Lattice_Constant[1], Lattice_Constant[2], alphe_calc, beta_calc, gamma_calc)
...
def Decompose_CIF(CIFfile):
    ...
    def solver_vector():
        ...
        z2 = float(Vector_cell_info['_cell_length_c']) * ((math.cos(alphe) - math.cos(beta) * math.cos(gamma)) / math.sin(gamma))
    ...
    Lattice_Constant = [float(Vector_cell_info['_cell_length_a']), float(Vector_cell_info['_cell_length_b']) , float(Vector_cell_info['_cell_length_c']) ]
    Crystalline_Angle = {'alphe': Vector_cell_info['_cell_angle_alpha'], 'beta': Vector_cell_info['_cell_angle_beta'] , 'gamma':Vector_cell_info['_cell_angle_gamma']}
    ...
    Volume = calculate_Volume(Lattice_Constant[0], Lattice_Constant[1], Lattice_Constant[2], float(Crystalline_Angle['alphe']), float(Crystalline_Angle['beta']), float(Crystalline_Angle['gamma']) )
    ...
    def Get_POS_info():
        ...
        for txt in file_txt.strip().split('\n'):
            ...
            if re.search("^_atom_site_label", ''.join(txt.split())):  # 定位atom_site_label，并向上和向下寻找结束语句
                lable_squence = count
                First_line_txt = linecache.getline(CIFfile, 1).split()
        ...
        for grepinitxt in ciffile_txt.strip().split('\n'):
            ...
            if re.search('^_atom_site_label', ''.join(line_read)): 
                ...
                while True:
                    ...
                    if re.search('loop_', ''.join(Toend_txt)) or not Toend_txt and not linecache.getline(CIFfile, Toend + 1).strip().split():
                        break
                break
...
ini_t = time.time()
print(Decompose_CIF('qekit\\method\\test\\CdTe.cif'))
print(f'用时：{time.time() - ini_t}秒')
# print(Decompose_POSCAR('qekit\\method\\test\\POSCAR_lsmo'))
